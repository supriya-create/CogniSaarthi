...
translated caregiver dashboard, account deletion — and **Memory Lane**, the
spaced-retrieval feature this phase is really about. It is not the full
...
| **Memory Lane** | Spaced retrieval + errorless learning over the person's own photographs. A practice schedule — never a treatment, a diagnosis, or a measurement of memory. See [docs/memory-lane.md](docs/memory-lane.md) |
...
| `npm run db:seed:demo` | Builds a complete synthetic family with a month of history |
...
The fastest route is the demo seed, which builds one synthetic family with
enough history behind it that every screen has something real to show:

```bash
npm run db:seed:demo
```

It prints the caregiver sign-in and the connection code. Everything it creates
is synthetic — the address is on `.example`, a domain reserved so it can never
belong to anybody, and the photographs are flat illustrations generated by the
script rather than pictures of anyone. Re-running it rebuilds the same family,
including the files on disk.

[`docs/demo.md`](docs/demo.md) is the twelve-minute walkthrough it is built for.
...
docs/           memory-lane, demo, offline-first, privacy-and-consent,
                research-data, caregiver-offline
```
...
The rule those follow: unused tables are scaffolding, not architecture, and
they get added with the features that fill them. `Reminder` arrived with Phase
4, `PersonalMemory` with Phase 3, `MemoryRecallEvent` with Phase 7's recall
tracking, and `MemoryAudio` with Phase 8's familiar voice — each one when
something actually wrote to it. There is still no `VoiceSession` table, because
nothing would put a row in it.
...
- **Memory Lane is a practice schedule, not a treatment.** It shows a
  personally meaningful photograph at widening intervals and never lets
  somebody sit with a wrong answer. It does not treat, reverse or diagnose
  anything, no screen suggests it does, and a test runs every string it can
  render — in all three languages — through the forbidden-phrase guard. The
  caregiver's retention states (`HOLDING`, `NEEDS_REINFORCEMENT`, …) describe
  the schedule, never the person.
...
- Memory Lane's schedule is derived by folding a memory's recall history. The
  snapshot carries the most recent 400 events, which is far more than the
  eight-step schedule can traverse — but a device that has been offline long
  enough to miss more than that would rebuild its schedule from the tail rather
  than the whole history. In practice the tail is what determines the state,
  since the step only ever moves one at a time.
- A caregiver's voice recording is played back by the browser's own audio
  element. Nothing transcodes it, so a recording made in Safari and played in
  a browser that cannot decode `audio/mp4` falls back to text-to-speech rather
  than failing loudly.
